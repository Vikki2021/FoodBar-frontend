...
export default function Navbar() {
  ...
  const getNavLinkStyle = (isActive) => ({
    transition:
      "color 0.3s ease-in-out, font-weight 0.3s ease-in-out, text-shadow 0.3s ease-in-out, border-bottom 0.3s ease-in-out, padding-bottom 0.3s ease-in-out",
          ...
          >
            <ul className="navbar-nav mx-auto mb-2 mb-lg-0">
              <li className="nav-item">
                <Link
                  ...
                  onMouseOut={(e) =>
                    handleNavLinkMouseOut(e, location.pathname === "/")
                  }
                >
                  Home
                </Link>
              ...
              <li className="nav-item">
                <Link
                  className="nav-link fs-5 mx-4"
                  aria-current={
                    location.pathname === "/aboutus" ? "page" : undefined
                  }
                  to="/aboutus"
                  style={getNavLinkStyle(location.pathname === "/aboutus")}
                  onMouseOver={handleNavLinkMouseOver}
                  onMouseOut={(e) =>
                    handleNavLinkMouseOut(e, location.pathname === "/aboutus")
                  }
                >
                  About Us
                </Link>
              ...
              {localStorage.getItem("token") ? (
                <li className="nav-item">
                  <Link
                    className="nav-link fs-5 mx-4"
                    aria-current={
                      location.pathname === "/myorder" ? "page" : undefined
                    }
                    to="/myorder"
                    style={getNavLinkStyle(location.pathname === "/myorder")}
                    onMouseOver={handleNavLinkMouseOver}
                    onMouseOut={(e) =>
                      handleNavLinkMouseOut(e, location.pathname === "/myorder")
                    }
                  >
                    My Orders
                  </Link>
            ...
            {!localStorage.getItem("token") ? (
              <div className="d-flex ms-auto">
                <Link
                  className="btn btn-outline-warning mx-1 rounded-pill fw-bold"
                  to="/login"
                  style={{
                ...
                <Link
                  className="btn btn-warning mx-1 rounded-pill fw-bold"
                  to="/register"
                  style={{
                    transition: "all 0.3s ease-in-out",
                    fontFamily: "Montserrat, sans-serif",
