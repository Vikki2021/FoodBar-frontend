    ...
    try {
      ...
      if (storedUser) {
        try {
          ...
          email =
            typeof parsed === "object" && parsed !== null && parsed.email
              ? parsed.email
              : String(parsed);
        } catch (e) {
          email = storedUser;
          ...
          if (Array.isArray(order.order_data)) {
            order.order_data.forEach((itemGroupArray, groupIndex) => {
              ...
              if (!Array.isArray(itemGroupArray)) {
                console.warn(
                  `Skipping non-array element in order_data for order ID ${order._id}:`,
                  itemGroupArray
                );
    ...
    >
      ...
      <div className="container mt-5 py-5">
        <div className="mb-4">
          <Link
            to="/menu"
            className="btn btn-link text-warning text-decoration-none fs-5"
          >
            ← Back to Home
          </Link>
        </div>
              ...
              <div className="col-12" key={itemGroup.key}>
                ...
                >
                  <div className="card-header border-bottom border-warning d-flex justify-content-between align-items-center py-3 px-4">
                    <h5 className="mb-0 text-warning">Purchase #{index + 1}</h5>
                        ...
                        >
                          <div>
                            ...
                            <span className="text-muted">
                              ({item.size ? item.size : "N/A Size"}, Qty:{" "}
                              {item.quantity})
                            </span>
                          </div>
                  ...
                  <div className="card-footer d-flex justify-content-between align-items-center bg-dark border-top border-warning py-3 px-4">
                    <strong className="text-warning fs-5">
                      Total for this Purchase:
                    </strong>
                    <span className="text-success fs-4 fw-bold">
                      ₹{itemGroup.groupTotal}
                    </span>
                  </div>
                </div>
              </div>
